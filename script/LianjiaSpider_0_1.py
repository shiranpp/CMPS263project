'''
    LianjiaSpier-District Capturer
    Author:Ryan Shi
    For CMPS263 project
    ...
    try:
        for distitem in dist:
            ...
            for mdistitem in mdist_list:
                mdist.append(
                    {'id': str(mdistitem.get('href').split('/')[2]), 'name': mdistitem.text, 'dist_id': distitem.id,
                     'cood': ''})
            session.execute(MDist.__table__.insert().prefix_with(' OR IGNORE'), mdist)
    except:
        print "time out or invaild network, please change network rerun again"
